...
$breadcrumb->add($lng['input_messages']);
...
$tpl_data['pagination'] = ($total > $kmess ? functions::display_pagination('input?page=', $start, $total, $kmess) : '');
$tpl_data['items'] = [];
if ($total) {
    ...
    while ($row = mysql_fetch_assoc($req)) {
        ...
        $last_msg = mysql_fetch_assoc(mysql_query('SELECT `text`, `time`, `read`
            FROM `cms_mail`
            WHERE `from_id`="' . $user_id . '"
            AND `user_id` = "' . $row['id'] . '"
            AND `delete` != "' . $user_id . '"
